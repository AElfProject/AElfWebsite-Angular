...
;(function($){
  ...
  $('body').on('click','.mobile-tool-menu',function () {
    let is_open = $(this).parent('li').hasClass('mobile-active');
    if(is_open){
      $(this).parent('li').css('margin-bottom',0).removeClass('mobile-active').find('i').removeClass('fa-angle-up').addClass('fa-angle-down')
      $('.mobile-tool-menu ul').hide();
    }else{
      $(this).parent('li').css('margin-bottom',160).addClass('mobile-active').find('i').removeClass('fa-angle-down').addClass('fa-angle-up')
      $('.mobile-tool-menu ul').show();
    }
  });

  $('body').on('click','.mobile-language-menu',function () {
    let is_open = $(this).parent('li').hasClass('mobile-active');
    if(is_open){
      $(this).parent('li').removeClass('mobile-active').find('i').removeClass('fa-angle-up').addClass('fa-angle-down')
      $('.mobile-language-menu ul').hide();
    }else{
      $(this).parent('li').addClass('mobile-active').find('i').removeClass('fa-angle-down').addClass('fa-angle-up')
      $('.mobile-language-menu ul').show();
    }
  });
  ...
  $('body').on('click','.mobile-nav li',function () {
...
window.initSkrollr = function () {
	if (!device.android() && !device.ios()) {
	  ...
	  if (window.skrollrInstance.isMobile()) {
...
$(window).load(function(){
  window.initSkrollr();
  let timeout = null;
  $("#data-container").bind("DOMNodeInserted", function () {
    if (timeout) {
      clearTimeout(timeout);
    }
    timeout = setTimeout(() => {
      if (!device.android() && !device.ios()) {
        window.skrollrInstance.refresh();
      }
    }, 0);
  });
});

function renderEarthCanvas() {
  // var bli = 1;
	// var bli2 = 1;
	// if(device.mobile()){
	// 	bli = 6;
...
function loadingClose() {
  var initLoadingDom = $('.init-loading');
