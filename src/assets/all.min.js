...
$(function(){
	...
	if($('.section-6').length){
		...
		var prevBtn = $('.section-6').find('.btn-prev');
		var nextBtn = $('.section-6').find('.btn-next');
		var curBoxBtn = $('.section-6').find('.btn-current');
		var prevBox = $('.section-6').find('.prev-value');
		var nextBox = $('.section-6').find('.next-value');
		var update = function(){
			curBox.find('h3').fadeOut(200);
			curBox.find('p').fadeOut(200);
			setTimeout(function(){
				curBoxBtn.html(allTxt.eq(curIndex).find('span').html());
				curBox.find('h3').html(allTxt.eq(curIndex).find('span').html());
				curBox.find('p').html(allTxt.eq(curIndex).find('p').html());
				curBox.find('h3').fadeIn(200);
				curBox.find('p').fadeIn(200);
			}, 200);

			if(curIndex > 0){
				prevBox.find('h4').fadeOut(200);
				setTimeout(function(){
					prevBtn.html(allTxt.eq(curIndex - 1).find('span').html());
					prevBox.find('h4').html(allTxt.eq(curIndex - 1).find('span').html());
					prevBox.find('h4').fadeIn(200);
				}, 200);
			}else {
				prevBtn.html('');
			...
			if(curIndex < len){
				nextBox.find('h4').fadeOut(200);
				setTimeout(function(){
					nextBtn.html(allTxt.eq(curIndex + 1).find('span').html());
					nextBox.find('h4').html(allTxt.eq(curIndex + 1).find('span').html());
					nextBox.find('h4').fadeIn(200);
				}, 200);
			}else{
				nextBtn.html('');
		...
		curBox.bind('mouseenter', function(){
			clearInterval(time);
		});
		...
		curBox.bind('mouseleave', function(){
			time = setInterval(function(){
				nextFn();
			}, 2000);
		});
	...
	/*if($('#particles3').length){
		particlesJS('particles3', {
			particles: {
				...
				line_linked: {
					enable_auto: true,
					distance: ($(window).width() <= 768) ? 60 : 200,
					color: '#fff',
					opacity: 1,
					width: 1,
					condensed_mode: {
...
;$(function(){
	...
	if(true){
		...
		function addClouds() {
			var spGeo = new THREE.SphereGeometry(400,30,30);
			var cloudsTexture = THREE.ImageUtils.loadTexture( "dist/images/earth_clouds_1024.png" );
