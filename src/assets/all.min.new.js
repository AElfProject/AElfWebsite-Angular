...
;$(function(){
	...
	//}
	//if(!device.android()){
    // https://dzone.com/articles/html5-render-open-data-3d
	if(true){
		// couple of constants
		var POS_X = 1.8;
		var POS_Y = 0.5;
		...
		function addDensity(data) {
			...
			for (var i = 0 ; i < data.length-1 ; i++) {
				...
				var cube2 = new THREE.Mesh(new THREE.BoxGeometry(1 * bli2,1 * bli2,1 * bli2, 1 , 1, 1), cubeMat);
				cube2.position = position2;
				cube2.lookAt( new THREE.Vector3(0,0,0) );
                // THREE.GeometryUtils.merge(geom, cube2);
                // console.log('--------------------', Object.assign({}, geom));
				geom.mergeMesh(cube2);
                // console.log('---------222-----------', Object.assign({}, geom));
                // THREE.GeometryUtils.merge(geom, cube2);

				if(1+value/30/2 * bli2 >= 8 * bli2){
                    // var cube = new THREE.Mesh(new THREE.CubeGeometry(1 * bli2,1 * bli2,1+value/30/2 * bli2,1,1,1,cubeMat3));
					var cube = new THREE.Mesh(new THREE.BoxGeometry(1 * bli2,1 * bli2,1+value/30/2 * bli2,1,1,1), cubeMat3);
		...
		//	var planetTexture = THREE.ImageUtils.loadTexture( "dist/images/world-big-2-grey.jpg" );
